...
# Anchor data-file paths to this script's directory so they resolve correctly
# regardless of where the server's working directory is.
_DUAL_DIR = _os.path.dirname(_os.path.abspath(__file__))
...
def calcEigen(RNA,vertexOrder): # S.J. 07/05/2018 - removing the last argument as that is not needed
    ...
    if len(RNA.Helices)==1:
        ...
        eigenfile = _os.path.join(_DUAL_DIR, "DualEig", "%dEigen" % (len(RNA.Helices)-1))
        adjMatfile = _os.path.join(_DUAL_DIR, "DualAdj", "V%dAdjDG" % (len(RNA.Helices)-1))
